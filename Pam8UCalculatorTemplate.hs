...
-- Since 2020-03-19
module Pam8UCalculatorTemplate where
...
import Pam8USignatureInterpreter
...
calculatorTemplate ::
  (Show valuedomain, Read valuedomain) =>
  Signature ->
  FunModel valuedomain ->
  TestData valuedomain ->
  ValueType valuedomain ->
  IO ()
          ...
          case readMaybe str of
            ...
            Just stmt@(SetVar vname expr)
              | [] <- allDeclared state expr,
                (res, []) <- typeCheckExpr intrinsics tenv valtyp expr -> do
            ...
            Just stmt@(AssVar vname expr)
              | isDeclared vname state,
                [] <- allDeclared state expr,
                (res, []) <- typeCheckExpr intrinsics tenv valtyp expr,
                Just res' <- lookup vname tenv -> do
                  if res == res'
                    then outputStr ""
    ...
    checkExpression state intrinsics tenv valtyp expr = do
      ...
      if undeclared == ("", [])
        then outputStr ""
          ...
          outputStrLn $
            "Syntax error, undeclared functions/variables: "
